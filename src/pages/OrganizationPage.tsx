...
export default function OrganizationPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: error => toast.error(
      `${getErrorMessage(error)} Data yang masih dipakai karyawan tidak dapat dihapus.`,
    ),
  });
  ...
  return (
    <>
      <PageHeader
        ...
        description="Susun hierarki divisi, departemen, dan jabatan yang digunakan pada data karyawan dan analitik payroll."
        actions={<Button onClick={() => openForm()}><Plus className="size-4" /> Tambah {title}</Button>}
      />
      ...
      <Card className="mt-5 overflow-hidden">
        {organization.isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="data-table min-w-[760px]">
              <thead>
                <tr>
                  <th>Nama</th>
                  {tab !== 'divisions' && <th>Induk Organisasi</th>}
                  <th>Deskripsi</th>
                  <th>Dibuat</th>
              ...
              <tbody>
                {currentData.map(item => {
                  ...
                  return (
                    <tr key={item.id}>
                      <td className="font-bold text-slate-900 dark:text-white">{item.name}</td>
                      {tab !== 'divisions' && (
                        <td>
                          {tab === 'departments'
                            ? division?.name ?? '—'
                      ...
                      <td className="max-w-md text-slate-500 dark:text-slate-400">{item.description || '—'}</td>
                      <td>{new Date(item.created_at).toLocaleDateString('id-ID')}</td>
                      <td>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" onClick={() => openForm(item)} aria-label={`Edit ${item.name}`}>
                          ...
                          <Button
                            ...
                            onClick={() => setDeleteTarget({ tab, id: item.id, name: item.name })}
          ...
          <EmptyState
            ...
            description={`Tambahkan ${title.toLowerCase()} pertama untuk mulai menyusun struktur perusahaan.`}
      ...
      <ConfirmDialog
        ...
        description={`${deleteTarget?.name ?? ''} akan dihapus. Tindakan ditolak bila data masih digunakan oleh karyawan atau relasi lain.`}
