...
export async function buildPayslipPdf(row:PayrollDetailRow,options:PdfOptions={}) {
  ...
  if(logo) {
    try { doc.addImage(logo.dataUrl, logo.format, 18, 18, 22, 22); }
    catch { doc.setFillColor(255,255,255);doc.circle(29,29,10,'F'); }
  }
  doc.setTextColor(255,255,255);
  doc.setFont('helvetica','bold');doc.setFontSize(17);doc.text(companyName,46,26);
  doc.setFont('helvetica','normal');doc.setFontSize(9);doc.text('SLIP GAJI KARYAWAN',46,33);
  ...
  autoTable(doc,{
    ...
    body:[...income.map(([label,value])=>[label,formatCurrency(Number(value))]),
      ['Total Pendapatan',formatCurrency(row.total_income)]],
    theme:'grid',headStyles:{fillColor:[79,70,229],fontStyle:'bold'},
    styles:{fontSize:9,cellPadding:2.4},columnStyles:{1:{halign:'right'}},
    didParseCell:(data)=>{
      if(data.section==='body'&&data.row.index===income.length) {
        data.cell.styles.fontStyle='bold';data.cell.styles.fillColor=[238,242,255];
      }
    }
  });
  ...
  autoTable(doc,{
    startY:afterIncome,head:[['POTONGAN','JUMLAH']],
    body:[...deductions.map(([label,value])=>[label,formatCurrency(Number(value))]),
      ['Total Potongan',formatCurrency(row.total_deduction)]],
    theme:'grid',headStyles:{fillColor:[225,29,72],fontStyle:'bold'},
    styles:{fontSize:9,cellPadding:2.4},columnStyles:{1:{halign:'right'}},
    didParseCell:(data)=>{
      if(data.section==='body'&&data.row.index===deductions.length) {
        data.cell.styles.fontStyle='bold';data.cell.styles.fillColor=[255,241,242];
      }
    }
  });
  const y=Math.min(244,(doc as unknown as {lastAutoTable:{finalY:number}}).lastAutoTable.finalY+7);
  doc.setFillColor(236,253,245);doc.roundedRect(14,y,132,26,4,4,'F');
  doc.setTextColor(6,95,70);doc.setFont('helvetica','bold');doc.setFontSize(10);
  doc.text('TOTAL DITERIMA',20,y+9);doc.setFontSize(18);
...
export async function exportPayslipsZip(
  ...
  for(let i=0;i<rows.length;i+=1) {
    const doc=await buildPayslipPdf(rows[i],options);
    zip.file(`${safeFileName(rows[i].slip_number)}-${safeFileName(rows[i].employee_name)}.pdf`,
      doc.output('arraybuffer'));
    options.onProgress?.(((i+1)/rows.length)*90);
  }
  const blob=await zip.generateAsync({type:'blob'},meta=>options.onProgress?.(90+meta.percent*.1));
  downloadBlob(blob,`slip-gaji-${new Date().toISOString().slice(0,10)}.zip`);
  await logActivity({
    action:'EXPORT',entityType:'payslip',
    description:`Export ZIP ${rows.length} slip`,metadata:{count:rows.length}
  });
}
function exportRows(rows:PayrollDetailRow[]) {
  return rows.map(row=>({
    NomorSlip:row.slip_number,Periode:row.period,NIK:row.nik,Nama:row.employee_name,
    Jabatan:row.position_name,Divisi:row.division_name,Departemen:row.department_name,
    GajiPokok:row.basic_salary,TunjanganTetap:row.fixed_allowance,
    TunjanganTidakTetap:row.variable_allowance,Bonus:row.bonus,Insentif:row.incentive,
    Lembur:row.overtime,THR:row.thr,Potongan:row.deduction,Kasbon:row.loan,
    BPJS:row.bpjs,Pajak:row.tax,TotalPendapatan:row.total_income,
    TotalPotongan:row.total_deduction,TotalBersih:row.net_salary,Status:row.status
  }));
}
